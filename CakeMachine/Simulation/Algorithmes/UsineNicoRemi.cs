...
internal class UsineNicoRemi : Algorithme
{
    /// <inheritdoc />
    ...
    private class OrdreProduction
    {
        ...
        public async IAsyncEnumerable<GâteauEmballé> ProduireAsync(Usine usine)
        {
        ...
        [EnumeratorCancellation] CancellationToken token)
    {
        ...
        await foreach (var gâteauEmballé in ligne.ProduireAsync(usine).WithCancellation(token))
            if (!gâteauEmballé.EstConforme) usine.MettreAuRebut(gâteauEmballé);
            else yield return gâteauEmballé;
    }
}